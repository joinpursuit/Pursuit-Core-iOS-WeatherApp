...
//    static func getCityAndState(from zipCode: String, completionHandler: @escaping (Error?, String?) -> Void) {
//        let geocoder = CLGeocoder()
//        DispatchQueue.global(qos: .userInitiated).async {
//            geocoder.geocodeAddressString(zipCode){(placemarks, error) -> Void in
//                DispatchQueue.main.async {
//                    if let placemark = placemarks?.first, let cityName = placemark.locality, let stateName = placemark.administrativeArea {
//                        completionHandler(nil, "\(cityName),\(stateName)")
//                    } else {
//                        completionHandler(error, nil)
//                    }
//                }
